...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        std::ofstream file("/home/fmthhadmin/Debayan/ZebraTrack/video-data/test.data", std::ios::out | std::ios::binary | std::ios::app);
        ...
        while ( camera.IsGrabbing())
        {
            ...
            if (ptrGrabResult->GrabSucceeded())
            {
                ...
                for (int row = 0; row < yuvImage.rows; row++)
                {

                    unsigned char* p = yuvImage.ptr(row);
                    for (int col = 0; col < yuvImage.cols; col++)
                    {
                ...
                //namedWindow( "OpenCV Display Window", WINDOW_AUTOSIZE);//AUTOSIZE //FREERATIO
